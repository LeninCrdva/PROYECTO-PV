...
public class ControladorFactura {
    ...
    private void cargaDet(String id){
        ...
        DefaultTableModel mTabla=(DefaultTableModel) vista.getTblDetalle().getModel();
        mTabla.setNumRows(0);   
...
//                String sql="SELECT id_enc,idcliente_enc,idempl_enc,fecha_enc,total_enc,estado_enc FROM enc_factura WHERE id_enc="+Integer.parseInt(vista.getTblFactura().getValueAt(vista.getTblFactura().getSelectedRow(), 0).toString())+"";
//                ResultSet re=con.Consulta(sql);re.next();
                vista.getTxtCodigoE().setText(vista.getTblFactura().getValueAt(vista.getTblFactura().getSelectedRow(), 0).toString());
    ...
    private void ingresarModificarDetalle(int c){
        ...
        if (c==0) {
            try{
                if (modelo2.buscaDetalleF(vista.getTxtCodigoD().getText())==true) {
                    JOptionPane.showMessageDialog(null, "YA EXISTE UN DETALLE CON ESE ID");
                    return ;
                }
...
//            new ModeloDetalleFactura(Integer.parseInt(vista.getTxtCodigoD().getText()),Integer.parseInt(vista.getTxtCodigoE().getText()),Integer.parseInt(vista.getTxtServicio().getText()),Integer.parseInt(vista.getTxtReserva().getText()),vista.getTxtObservaciones().getText(),Double.parseDouble(vista.getTxtCosto().getText())).grabarDetalleF();
            new ModeloDetalleFactura(Integer.parseInt(vista.getTxtCodigoD().getText()),Integer.parseInt(vista.getTxtCodigoE().getText()),2,Integer.parseInt(vista.getTxtReserva().getText()),vista.getTxtObservaciones().getText(),Double.parseDouble(vista.getTxtCosto().getText())).grabarDetalleF();
